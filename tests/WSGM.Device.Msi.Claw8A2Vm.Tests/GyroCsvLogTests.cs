...
public sealed class GyroCsvLogTests
{
    ...
    public async Task WritesInvariantDiagnosticRowAndHeader()
    {
        ...
        log.Write(new GyroCsvRow(
            ...
            PublishedAngularVelocity: new Vector3(0.25f, -0.125f, 1.25f),
        ...
        Assert.Contains(",0.75,-0.25,1.5,0.5,-0.125,0.25,0.25,-0.125,1.25,", lines[1], StringComparison.Ordinal);
    ...
    [Fact]
    public async Task LeavesTheOffsetColumnsEmptyBeforeTheFirstRestWindow()
    {
        using TemporaryDirectory temporary = new();
        string path = Path.Combine(temporary.Root, GyroCsvLog.FileName);
        GyroCsvLog log = GyroCsvLog.Create(path, 1024 * 1024);
        ...
        log.Write(new GyroCsvRow(
            ...
            0,
            0,
            new Vector3(0.75f, -0.375f, -0.145f),
            null,
            new Vector3(0.75f, -0.375f, -0.145f),
            Vector3.UnitZ));
        await log.DisposeAsync();
        ...
        string[] lines = await File.ReadAllLinesAsync(path);
        Assert.Contains(
            ",0.75,-0.375,-0.145,,,,0.75,-0.375,-0.145,",
            lines[1],
